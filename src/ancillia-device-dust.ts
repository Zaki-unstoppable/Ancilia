...
const DUST_MASS = 2.2e-9; // effective mass used for acceleration scaling
...
class DustParticle {
  ...
  respawn() {
    ...
    this.vel.set(0, 0, 0);
    this.stickTimer = 0.45 + Math.random() * 0.9;
    this.captured = false;
  ...
  step(dt: number, t: number) {
    ...
    if (this.stickTimer > 0) {
      this.stickTimer -= dt;
      this.mesh.scale.lerp(new THREE.Vector3(0.7, 0.5, 0.7), 0.4);
      this.mesh.position.copy(this.pos);
      return;
    }

    if (!this.captured && this.vel.lengthSq() === 0) {
      this.vel.set(
    ...
    const ax = (this.q * F.x) / DUST_MASS;
    const ay = (this.q * F.y) / DUST_MASS - GRAV;
    const az = (this.q * F.z) / DUST_MASS;
    ...
    const radial = new THREE.Vector2(this.pos.x, this.pos.z).length();
    const nearTrench = radial < TRENCH_RADIUS * 1.2;

    if (!insideHex(this.pos) || (nearTrench && this.pos.y < 0.04)) {
      this.captured = true;
      this.mesh.scale.lerp(new THREE.Vector3(0.18, 0.18, 0.18), 0.35);
      if (this.mesh.scale.length() < 0.35) this.respawn();
    }
...
export function initDeviceDustDemo(container: HTMLElement) {
 ...
 codex/enhance-ancillia-device-dust-visuals-epurzc
  // Layered dielectric rings to catch rim light
  const layers = new THREE.Group();
  const layerOffsets = [0.0035, 0.0045, 0.0055];
  ...
  coverGlass.material.onBeforeCompile = (shader) => {
    ...
    console.log('coverGlass dispersion shader compiled');
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <output_fragment>',
      `
        #include <output_fragment>
        float vAngle = abs(dot(normalize(vNormal), normalize(vViewPosition)));
        float spread = pow(1.0 - vAngle, 1.45) * chromaticSpread;
        vec3 tint = vec3(1.0 + spread * 0.55, 1.0 + spread * 0.08, 1.0 + spread * -0.55);
        gl_FragColor.rgb *= tint;
        gl_FragColor.rgb += vec3(0.02) * spread;
  ...
  const laneMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float rim = pow(1.0 - cosI, 2.4);
        vec3 shimmer = film * (0.65 + 0.35 * sin(phase * 0.5 + vec3(0.0, 2.0, 4.0)));
        vec3 color = mix(baseColor, shimmer, F) + rim * film * 0.35;
 ...
 codex/enhance-ancillia-device-dust-visuals-epurzc
    trenchMat.emissiveIntensity = 0.32 + 0.22 * Math.sin(t * 3.8);
